...
from src.multiprogressbars.bar_updater import BarUpdater
from src.multiprogressbars.helpers.graphics_widgets import ZoomingScrollArea, LabeledProgressBar, Menu
from src.multiprogressbars.helpers import ProcessHandler
from src.multiprogressbars.helpers.util import handle_mutex_and_catch_runtime
...
class MultibarCore(QtCore.QObject):
    ...
    def end_task(self, pid, exit_code=ProcessHandler.SUCESSFUL):
        if exit_code == ProcessHandler.CANCELLED:
            self.pbars[pid].set_color(LabeledProgressBar.StateCancelled)
            self.failed_tasks[pid] = ProcessHandler.CANCELLED
        if exit_code == ProcessHandler.EXCEPTION_RAISED:
            self.pbars[pid].set_color(LabeledProgressBar.StateException)
    ...
    def cancel_task(self, pid):
        ...
        if confirmed:
            self.end_task(pid, LabeledProgressBar.StateCancelled)
